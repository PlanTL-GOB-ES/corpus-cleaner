...
from corpus_cleaner.components.cleaner_component_mapper import CleanerComponentMapper, LegacyCleanerComponentMapper
import re
import argparse
import fasttext
import os
...
class PreFiltererConfig:
    ...
    no_head_filter: bool = False  # Filtering documents coming from a crawler (having a "heads" attribute) with
    ...
    target_langs: Union[Tuple[str], None] = None  # Target languages. PREVIUSLY: --lang-filter
    initial_lang_filter_threshold: float = 0.3  # 'If --lang-filter is set, minimum threshold for the initial lang
    # identifier.
    dictionary_filter_doc: Optional[str] = None  # Path to dictionary (plain text, one term per line of terms that should
    # not appear in a document). PREVIOUSLY: DO write dictionary_filter.
...
class PreFilterer(CleanerComponentMapper):
    ...
    def _build_transforms(self):
        ...
        if self._config.replace_urls:
            transforms.append(ReplaceURLsStringTransform(self._lang_chars))
...
class LegacyPreFilterer(LegacyCleanerComponentMapper):
