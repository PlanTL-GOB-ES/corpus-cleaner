...
def check_http_status(record):
    if HTTP_STATUS:
        return record.http_headers and record.http_headers.statusline[0] in ['2', '3']
    else:
        return True
        ...
        with open(path, 'rb') as stream:
            for record in ArchiveIterator(stream):
                if (record.rec_type) == 'response' and ('http' in record.content_type or
                        'html' in record.content_type or 'text/plain' == record.content_type) and check_http_status(record):
                    ...
                    if URI_NOT_COMPLETE:
                        uri = urljoin(os.path.basename(path).replace(".warc.gz", "").replace(".arc.gz", "") + '/', uri)
...
def write_data(path, data):
    output_file = os.path.splitext(os.path.basename(path))[0] + f'_{str(uuid4())[:8]}.json'
